...
apptitle="aerOS Server Edition Installer v1 Beta 1"
cpl=0
skipfont="0"
fspkgs=""
...
installmenu(){
	...
	options+=("${txtinstallaeros}" "pacstrap")
	options+=("${txtconfigaeros}" "")
...
installbase(){
	...
	sel=$(dialog --backtitle "${apptitle}" --title "${txtinstallaeroskernel}" --menu "" 0 0 0 \
		"${options[@]}" \
		3>&1 1>&2 2>&3)
	if [ "$?" = "0" ]; then
		pkgs+=" ${sel}"
	...
	sel=$(dialog --backtitle "${apptitle}" --title "${txtinstallaerosfirmwares}" --checklist "" 0 0 0 \
		"${options[@]}" \
		3>&1 1>&2 2>&3)
	if [ ! "$?" = "0" ]; then
		return 1
	...
	options+=("lvm2" "" off)
	options+=("dmraid" "" off)
	sel=$(dialog --backtitle "${apptitle}" --title "${txtinstallaerosfilesystems}" --checklist "" 0 0 0 \
		"${options[@]}" \
		3>&1 1>&2 2>&3)
	if [ ! "$?" = "0" ]; then
		return 1
	...
	if [[ ${cpl} == 1 ]]; then
		sed -i '/^\s*$/d' /tmp/aeros-custom-package-list
		sed -i '/^#/ d' /tmp/aeros-custom-package-list
		options=()
		while read pkg; do
			options+=("${pkg}" "" on)
		done < /tmp/aeros-custom-package-list
		sel=$(dialog --backtitle "${apptitle}" --title "${txtinstallaeroscustompackagelist}" --checklist "" 0 0 0 \
			"${options[@]}" \
			3>&1 1>&2 2>&3)
		if [ ! "$?" = "0" ]; then
			return 1
...
archchroot(){
	...
	echo "dialog is required for aeros, it will be installed as dep. Then it will removed when cleaning orphan."
	echo "arch-chroot /mnt pacman -S --asdeps --needed  dialog"
	arch-chroot /mnt pacman -S --asdeps --needed  dialog
...
archsethostname(){
	hostname=$(dialog --backtitle "${apptitle}" --title "${txtsethostname}" --inputbox "" 0 0 "aeros" 3>&1 1>&2 2>&3)
	if [ "$?" = "0" ]; then
...
archsystemdinstall(){
	...
	echo "sed -i \"s/aerOS/aerOS Fallback/\" /mnt/boot/loader/entries/arch-fallback.conf"
	...
	sed -i "s/aerOS/aerOS Fallback/" /mnt/boot/loader/entries/arch-fallback.conf
...
archrefindinstall(){
	...
	echo "echo \"\\\"aerOS         \\\" \\\"root=UUID=${rootuuid} rw add_efi_memmap\\\"\" > /mnt/boot/refind_linux.conf"
	echo "echo \"\\\"aerOS Fallback\\\" \\\"root=UUID=${rootuuid} rw add_efi_memmap initrd=/initramfs-linux-fallback.img\\\"\" >> /mnt/boot/refind_linux.conf"
	echo "echo \"\\\"aerOS Terminal\\\" \\\"root=UUID=${rootuuid} rw add_efi_memmap systemd.unit=multi-user.target\\\"\" >> /mnt/boot/refind_linux.conf"

	pacstrap /mnt refind-efi
	archchroot refindbootloaderinstall ${realrootdev}
	rootuuid=$(blkid -s UUID -o value ${realrootdev})
	echo "\"aerOS         \" \"root=UUID=${rootuuid} rw add_efi_memmap\"" > /mnt/boot/refind_linux.conf
	echo "\"aerOS Fallback\" \"root=UUID=${rootuuid} rw add_efi_memmap initrd=/initramfs-linux-fallback.img\"" >> /mnt/boot/refind_linux.conf
	echo "\"aerOS Terminal\" \"root=UUID=${rootuuid} rw add_efi_memmap systemd.unit=multi-user.target\"" >> /mnt/boot/refind_linux.conf
	pressanykey
}
...
archextrasmenu(){
	...
	options+=("vim" "" on)
	options+=("dhcpcd" "" on)
	options+=("tailscale" "" off)
	options+=("ssh" "" on)
	options+=("htop" "" on)
	options+=("neofetch" "" off)
	options+=("bpytop" "" off)
	options+=("xorg" "" off)
	...
	if [[ "${pkgs}" == *"networkmanager"* ]]; then
		...
		if [[ "${pkgs}" == *"xorg-xinit"* ]]; then
		pacstrap /mnt xf86-video-intel xf86-video-amdgpu xf86-video-qxl xorg xorg-xinit xterm
	fi
	pressanykey
}
...
loadstrings(){
	...
	txtinstallaeros="Install aerOS"
	txtinstallaeroskernel="Kernel"
	txtinstallaerosfirmwares="Firmwares"
	txtinstallaerosfilesystems="File Systems"
	txtinstallaeroscustompackagelist="Custom Package List"
	txtconfigaeros="Config aerOS"
	...
	txtefibootmgr="efibootmgr is required for EFI servers."
...
while (( "$#" )); do
	case ${1} in
		--help)
			...
			echo "-t | --test ghusername ghbranch   Test aeros script and language"
			exit 0
		;;
		-cpl | --custom-package-list)
			curl -L ${2} > /tmp/aeros-custom-package-list
			if [[ "$?" != "0" ]]; then
		...
		-t | --test)
			curl -L https://raw.githubusercontent.com/${2}/aeros/${3}/aeros > aeros
			sh aeros -tt ${2} ${3}
			exit 0
		;;
		-tt) baseurl="https://raw.githubusercontent.com/${2}/aeros/${3}";;
		-efi0) efimode=0;;
		-efi1)
			eficomputer=1
			efimode=1
